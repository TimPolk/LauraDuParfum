...
def compute_note_similarity(idx):
    query_levels = note_level_matrix[idx]
    query_present = query_levels >= 0
    ...
    cand_present = note_level_matrix >= 0
    both_present = query_present & cand_present

    # Pyramid distance between levels
    level_diffs = np.abs(note_level_matrix - query_levels)
    level_diffs = np.clip(level_diffs, 0, 2)

    # Map distance to proximity weight
    weights = PROXIMITY[level_diffs]
    weights = weights * both_present
...
def compute_gender_compat(idx):
    query = gender_types[idx]

    # Exact same gender category
    exact_match = (gender_types * query).sum(axis=1) == 1
...
# (TP) normalize so Euclidean distance in HDBSCAN approximates cosine geometry
